...
const init = async (name, { tag }) => {
  ...
  if (!projectPath) {
    projectPath = path.join(process.cwd(), 'useprint-starter');
  }
  ...
  fse.writeFileSync(
    ...
    templatePackageJson
      .replace(
        'INSERT_USEPRINT_COMPONENTS_VERSION',
        await getLatestVersionOfTag('@useprint/components', tag),
      )
      .replaceAll(
        'INSERT_USEPRINT_CLI_VERSION',
        await getLatestVersionOfTag('@useprint/cli', tag),
      )
      .replaceAll(
        'INSERT_USEPRINT_PREVIEW_VERSION',
        await getLatestVersionOfTag('@useprint/preview', tag),
      ),
    'utf8',
  );
  ...
  spinner.stopAndPersist({
    ...
    text: 'Useprint starter files ready',
  });
...
new Command()
  ...
  .description('The easiest way to get started with Useprint')
  .arguments('[dir]', 'Path to initialize the project')
  .option('-t, --tag <tag>', 'Tag of Useprint versions to use', 'latest')
  .action(init)
  .parse(process.argv);
